...
import { IssueStatus, IssueStatusDisplay, JIssue } from '@viacheslav/interface/issue';
import { FilterState } from '@viacheslav/project/state/filter/filter.store';
  ...
  ngOnInit(): void {
    combineLatest([this.issues$, this._filterQuery.all$])
      ...
      .subscribe(([issues, filter]) => {
        console.log('issues', issues, 'fiter', filter);
